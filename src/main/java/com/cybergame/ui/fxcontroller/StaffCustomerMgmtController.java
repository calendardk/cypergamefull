...
public class StaffCustomerMgmtController implements Initializable {
    ...
    private Timeline autoRefreshTimer; // 🔥 Timer cập nhật tự động
    ...
    private void setupAutoRefresh() {
        // Cứ 3 giây refresh danh sách 1 lần
    ...
    // Dừng timer khi tắt tab (nếu cần xử lý kỹ hơn)
    public void stopTimer() {
        if (autoRefreshTimer != null) autoRefreshTimer.stop();
    }

    private void loadData() {
        // Chạy background thread để không đơ giao diện khi query DB
        new Thread(() -> {
            ...
            for (Account accDB : dbList) {
                // Ưu tiên lấy dữ liệu realtime (Online - đang bị trừ tiền)
                Account accOnline = context.get(accDB.getUsername());
                displayList.add(accOnline != null ? accOnline : accDB);
            }
            
            // Cập nhật lên giao diện (Bắt buộc dùng Platform.runLater)
            Platform.runLater(() -> {
                // Giữ lại dòng đang chọn để không bị mất focus
    ...
    private void onUpdate() {
        ...
        // 🔥 CHẶN: Nếu khách đang Online -> Nhân viên KHÔNG ĐƯỢC sửa
        if (AccountContext.getInstance().isOnline(selected.getUsername())) {
            showAlert("Hạn chế", "Khách hàng đang Online!\nKhông thể SỬA thông tin lúc này để tránh lỗi hệ thống.");
            return;
        }
    ...
    private void onLockUnlock() {
        ...
        // 🔥 CHẶN: Nếu khách đang Online -> Nhân viên KHÔNG ĐƯỢC khóa
        if (AccountContext.getInstance().isOnline(selected.getUsername())) {
            showAlert("Hạn chế", "Khách hàng đang Online!\nKhông thể KHÓA tài khoản lúc này.\nHãy yêu cầu khách đăng xuất trước.");
            return;
        }
    ...
    private void onDelete() {
        ...
        // 🔥 CHẶN: Nếu khách đang Online -> CẤM XÓA
        if (AccountContext.getInstance().isOnline(selected.getUsername())) {
